...
const _sfc_main = /* @__PURE__ */ vueExports.defineComponent({
  ...
  setup(__props) {
    ...
    const postCode = vueExports.computed(() => codeParam.value);
    const checkHistory = () => {
      const saved = (void 0).getFloor?.(postCode.value);
      if (saved) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_CommentTree = _sfc_main$1;
      _push(`<div${serverRenderer_cjs_prodExports.ssrRenderAttrs(_attrs)}>`);
      if (__props.requireAuth && vueExports.unref(needLogin)) {
        ...
        if (vueExports.unref(showHistoryPrompt) && vueExports.unref(historyFloor)) {
          _push(`<div id="historyPrompt"><span>上次看到 <b>#${serverRenderer_cjs_prodExports.ssrInterpolate(vueExports.unref(historyFloor))}</b> 楼</span><button>去这里</button><span class="close">×</span></div>`);
        } else {
          _push(`<!---->`);
        }
        ...
        if (__props.showQr) {
          _push(`<div id="qrPopup" style="${serverRenderer_cjs_prodExports.ssrRenderStyle(vueExports.unref(qrVisible) ? null : { display: "none" })}">`);
        ...
        if (vueExports.unref(topicReady)) {
          _push(`<div class="content">${vueExports.unref(topicContent) ?? ""}</div>`);
        } else if (vueExports.unref(loadingTopic) && !vueExports.unref(topicContent)) {
        ...
        if (__props.showShareLink && vueExports.unref(shareUrl)) {
        ...
        if (vueExports.unref(repliesReady) && vueExports.unref(total) > 0) {
          ...
          _push(`</div></div>`);
        } else if (vueExports.unref(loadingReplies) && !vueExports.unref(replies).length && vueExports.unref(total) === 0) {
...
//# sourceMappingURL=TopicPage-GzBjKICW.js.map
