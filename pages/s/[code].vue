...
const fetchTopic = async () => {
  ...
  try {
    const res: any = await $fetch(`/api/public-topic/${rawId.value}`, { query: { _t: Date.now() } })
    if (res?.title) {
...
const fetchReplies = async (silent = false) => {
  ...
  try {
    const res: any = await $fetch(`/api/public-replies/${rawId.value}`, { query: { _t: Date.now() } })
...
.flash-highlight { animation: highlight-fade 3s ease-out; border-radius: 8px; }
.comment-bar{font-size: 0.75rem;font-weight: 400; margin-bottom: 5px; color: var(--meta);display:flex;align-items:center;}
.comment-bar>*{ overflow:hidden;white-space:nowrap;text-overflow:ellipsis; }
.reply-author{color:#aaa;font-size:0.8rem;}
.reply-txt{font-size: 1rem; color: var(--text); overflow-wrap: break-word; line-height: 1.6;}
#newMsgNotify{visibility:hidden; position:fixed;top:20px;left:50%;transform:translateX(-50%) translateY(-150%);background:#ff2c55;color:#fff;padding:10px 24px;border-radius:30px;font-size:14px;font-weight:600;z-index:101;cursor:pointer;transition:transform 0.5s;}
#newMsgNotify.show{transform:translateX(-50%) translateY(0);visibility:visible;}
#historyPrompt{position:fixed;top:20px;left:50%;transform:translateX(-50%);background:var(--bg);border:1px solid var(--border);color:var(--text);padding:10px 16px;border-radius:40px;font-size:14px;z-index:102;cursor:default;box-shadow:0 10px 30px rgba(0,0,0,0.1);display:flex;align-items:center;gap:12px;}
#historyPrompt button{background:#1d2129;color:#fff;border:none;border-radius:12px;padding:6px 14px;cursor:pointer;font-weight:600;}
#historyPrompt .close{color:var(--meta);font-size:18px;margin-left:5px;padding:0 5px;cursor:pointer;}
.loading{color:var(--meta);padding:20px 0;text-align:center;}
.comments-title{margin-top:2rem;font-weight:bold;margin-bottom:10px;font-size:1.1rem;border-bottom:1.5px solid var(--text);display:inline-block;padding-bottom:3px;}
.skeleton-block{padding:6px 0;}
.skeleton-line{height:14px;border-radius:8px;background:linear-gradient(90deg, rgba(0,0,0,0.05) 25%, rgba(0,0,0,0.12) 37%, rgba(0,0,0,0.05) 63%);background-size:400% 100%;animation:skeleton 1.4s ease infinite;margin:10px 0;}
.w-90{width:90%}.w-85{width:85%}.w-80{width:80%}.w-70{width:70%}.w-60{width:60%}
