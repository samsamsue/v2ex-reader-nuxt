<template>
  <div>
    ...
    <div id="jumpPrompt"><span>检测到链接</span><button id="jumpBtn">立即跳转</button></div>
    <div id="mainContent" class="box">
      <h2>V2EX Reader</h2>
      <div style="display:flex; gap:10px; margin-bottom:20px; margin-top:10px;">
        <input type="text" id="vUrl" placeholder="粘贴链接或直接输入数字ID" style="flex:1; margin:0;">
        <button onclick="manualJump()" style="padding:0 24px; border:none; border-radius:12px; background:#1d2129; color:#fff; font-weight:600; cursor:pointer; white-space:nowrap; transition:transform 0.1s;">跳转</button>
      </div>
      <button onclick="location.href='/all'" style="width:100%;padding:14px;border:none;border-radius:12px;background:var(--border);color:var(--text);font-weight:600;cursor:pointer;">查看全部列表</button>
    </div>
  </div>
</template>
